...
import type { EditorCategory, EditorDraft, EditorFieldErrors } from "./editor-shared";
import { prepareEditorSubmitPayload } from "./editor.js";
...
export type EditorWritePayload = EditorDraft;
...
export async function writeEditorContentFile({
  ...
  return buildEditorWriteResult({
    slug: payload.slug,
    category: payload.category,
    outputPath: path.relative(process.cwd(), absoluteOutputPath).replaceAll("\\", "/"),
  });
}
